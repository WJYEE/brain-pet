...
export const FOCUS_REAL_ROUNDS = 5
...
/**
 * Fixed 5-round sequence on a 6x6 (36-cell) grid. Placement density and
 * similarity climb together, round over round. Compressed from the original
 * 8-round curve (First Play length reduction) by thinning the middle rather
 * than truncating the tail — round 5 still lands on the same hardest
 * difficulty (36 placements/similarity 4) the old round 8 used, so the peak
 * difficulty a fast player can reach is unchanged. Values are a draft per
 * product decision; adjust here after Beta testing.
 */
export const FOCUS_DIFFICULTY_SEQUENCE: FocusDifficultyLevel[] = [
  { level: 1, totalPlacementCount: 18, similarityLevel: 1 },
  { level: 2, totalPlacementCount: 24, similarityLevel: 1 },
  { level: 3, totalPlacementCount: 28, similarityLevel: 2 },
  { level: 4, totalPlacementCount: 32, similarityLevel: 3 },
  { level: 5, totalPlacementCount: 36, similarityLevel: 4 },
]

/** Per-round weight for Weighted Accuracy — harder rounds count slightly more. Index matches FOCUS_DIFFICULTY_SEQUENCE (re-spread across 5 entries, was 8). */
export const FOCUS_DIFFICULTY_ACCURACY_WEIGHTS = [1.0, 1.05, 1.1, 1.15, 1.2]
 ...
 */
export const FOCUS_ROUND_TIME_LIMIT_MS = [5000, 4500, 4000, 3800, 3500]

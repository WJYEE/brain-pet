...
/**
 * 37 hand-verified static questions (spec: "정적 문제 은행을 사용" — never
 * runtime-random-generated; 13 easy / 12 normal / 12 hard, all above the
 * "최소 36개, 난이도별 최소 12개" floor). Every entry's `explanation` was
 * checked against its `sequence`/`answer` by script before being frozen
 * here, `choices` always contains `answer` exactly once with no duplicate
 * values, and the blank is always the sequence's last (and only) `null`.
 */
 ...
 * 않게"), ramping easy -> normal -> hard. Choice order is reshuffled per
 * pick even though the underlying bank entry is frozen, so the correct
 * choice's position varies session to session.
 */
export function pickNumberPatternSession(questionCount = 8): NumberPatternQuestion[] {
  const difficultyForIndex = (i: number): 'easy' | 'normal' | 'hard' => {
  ...
  for (let i = 0; i < questionCount; i++) {
    ...
    const pool = noAnswerClash.length > 0 ? noAnswerClash : byDifficultyUnused.length > 0 ? byDifficultyUnused : NUMBER_PATTERN_QUESTIONS.filter((q) => !usedIds.has(q.id))
